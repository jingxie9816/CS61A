...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        eval_procedure = scheme_eval(first, env)
        eval_operands = rest.map(lambda expr: scheme_eval(expr, env))
        return scheme_apply(eval_procedure, eval_operands, env)
        # END PROBLEM 4
    ...
    def apply(self, args, env):
        ...
        def helper(args):
            if args is nil:
                ...
                return [args.first] + helper(args.rest)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        if is_false_primitive(eval_exp):
            return False
    while expressions.rest is not nil:
        eval_exp = scheme_eval(expressions.rest.first, env)
        if is_true_primitive(eval_exp):
            expressions = expressions.rest
        else:
            return False
    return eval_exp
