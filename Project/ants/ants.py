...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if isinstance(place.ant, ContainerAnt) and place.ant.can_contain(self) == True:
                Insect.add_to(self, place)
                place.ant.contain_ant(self)
            elif isinstance(self, ContainerAnt) and self.can_contain(place.ant) == True:
                pre_ant = place.ant
                place.ant = self
                Insect.add_to(self, place)
                self.contain_ant(pre_ant)
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        rg = 0
        while check_place is not None and check_place is not beehive:
            if self.min_range <= rg <= self.max_range:
                if rANTdom_else_none(check_place.bees) is not None:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        bees_copy1 = self.place.bees[:]    #***** Why the bees_copy will change? becuase b changes, so the element itself changes.
        
        for b in bees_copy1:
            Insect.reduce_armor(b, amount)  # the elements number in bees_copy will not change, but the armor of the elements will change
#        print("DEBUG:", bees_copy1)

        bees_copy2 = self.place.bees[:]     # When bee dies, the bee will be removed from self.place, so copy a new list of bees, otherwise, the following for loop will fail. 
        Ant.reduce_armor(self, amount)
#        print("DEBUG:", bees_copy2)

        if self.place is None:   # Means the ant died, (armor <= 0)
            for b in bees_copy2:
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.queenant_counter == 0:
            self.real_queen = True
        
        QueenAnt.queenant_counter += 1   # Class attribute to record how many queen ants have been created
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self.real_queen is True:
            ScubaThrower.action(self, gamestate)

            place_behind = self.place.exit
            while place_behind is not None:
                if place_behind.ant is not None and place_behind.ant.buffed == False:
                    place_behind.ant.damage *= 2
                    place_behind.ant.buffed = True
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        return self.contained_ant == None and isinstance(other, ContainerAnt) is False
...
def make_slow(action, bee):
    ...
    def slow(gamestate):
        if gamestate.time % 2 == 0:
            ...
            print('DEBUG:', 'gamestate', gamestate)
    return slow
...
def make_scare(action, bee):
    ...
    def scare(gamestate):
        ...
        if bee.armor > 0 and destination is not None:
...
def apply_status(status, bee, length):
    ...
    def action(gamestate):
        nonlocal length
        if length > 0:
            status(bee.action, bee)(gamestate)
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        if target:
            ...
            print("DEBUG:", 'target', target, 'target armor', target.armor)
