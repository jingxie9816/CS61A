...
def add_trees(t1, t2):
    ...
    """
    "*** YOUR CODE HERE ***"
    if not t1:   #??  not None = True, when it comes to the newly added branch- None, it just return the value of the other tree
        return t2
    if not t2:   #??
        return t1
    ...
    if len(branches(t1)) < len(branches(t2)):
        t1_branches += [None for i in range(len(branches(t1)),len(branches(t2)))]
    elif len(branches(t1)) > len(branches(t2)):
        t2_branches += [None for i in range(len(branches(t2)),len(branches(t1)))]
...
def build_successors_table(tokens):
    ...
    for word in tokens:
     #   if prev not in table:
      ...
      #      table[prev] = [word]  
        "*** YOUR CODE HERE ***"
        table[prev]= table.get(prev,[]) + [word]   # OR: table[prev].append(word)
        prev = word
    return table
